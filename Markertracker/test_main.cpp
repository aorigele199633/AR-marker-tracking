...
// Added in Exercise 9 - Start *****************************************************************
	float resultMatrix_0005[16];
	float resultMatrix_0041[16];
	for(int i=0; i<markers.size(); i++){
		...
		if(code == 0x0005) {
			...
			for(int j=0; j<16; j++)
				resultMatrix_0041[j] = markers[i].resultMatrix[j];
		}
	}
	...
	for (int x=0; x<4; ++x)
		for (int y=0; y<4; ++y)
			resultTransposedMatrix[x*4+y] = resultMatrix_0041[y*4+x];
// Added in Exercise 9 - End *****************************************************************
